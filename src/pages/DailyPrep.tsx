...
export default function DailyPrep() {
  ...
  const [pattern, setPattern] = useState<PatternKey>('deep_squat');
  ...
  // Auto-select worst pattern from latest FMS when accessed via clientId.
  useEffect(() => {
    let cancelled = false;
    if (!clientId) return;
    (async () => {
      ...
      if (allowed.includes(priority.patternKey as PatternKey)) {
        setPattern(priority.patternKey as PatternKey);
      }
    })();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display font-bold text-2xl">Daily Prep</h1>
        <p className="text-sm text-muted-foreground">
          Protocolli quotidiani basati sulla priorità FMS e sul focus dell'allenamento.
        </p>
      </div>

      <div className="surface-card p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
